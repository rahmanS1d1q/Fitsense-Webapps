...
export default function TrainerMonitoringPage() {
  ...
  // Load members + active sessions + today's assignments
  const loadData = useCallback(async () => {
    if (!companyId) return;
    const today = new Date().toISOString().slice(0, 10);
    const [membersRes, sessionsRes, assignmentsRes] = await Promise.all([
      apiGet(`/companies/${companyId}/members`),
      apiGet(`/companies/${companyId}/sessions/active`),
      apiGet(
        `/companies/${companyId}/assignments?status=pending&date=${today}`,
      ),
    ]);
    if (membersRes.ok) {
    ...
    if (assignmentsRes.ok) {
      ...
      (assignmentsRes.data.assignments ?? []).forEach(
        (a: { member_id: string; workout_name: string }) =>
          aMap.set(a.member_id, a.workout_name),
      );
          ...
          >
            {visibleMembers.map((m) => (
              <MemberCard
                ...
                assignmentName={assignmentMap.get(m.id) ?? null}

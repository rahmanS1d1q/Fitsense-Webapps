...
export default function WorkoutsListPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Hapus workout "${name}"?`)) return;
    const companyId = getCompanyId();
    const res = await fetch(
      `${API_URL}/companies/${companyId}/workouts/${id}`,
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc" }}>
      ...
      <div style={{ padding: "28px 24px", maxWidth: 900, margin: "0 auto" }}>
        ...
        >
          ...
          <button
            onClick={() => router.push("/dashboard/owner/workouts/new")}
            style={{
              padding: "10px 20px",
                ...
                {filtered.map((w) => (
                  <tr key={w.id} style={{ borderTop: "1px solid #f1f5f9" }}>
                    ...
                    <td style={{ padding: "12px 16px", textAlign: "right" }}>
                      <button
                        onClick={() =>
                          router.push(`/dashboard/owner/workouts/${w.id}/edit`)
                        }
                        style={{
                          padding: "4px 12px",
                          background: "#eff6ff",
